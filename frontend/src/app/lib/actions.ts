...
export async function addWord(formData: FormData) {
    const word = formData.get('word');
        ...
        if (res.ok) {
        const transRes = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/translate`)
...
export async function selectAnswer(wordId: number, translation: string) {
    const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/submit-answer/${wordId}/${translation}`, {
        method: 'POST'
    });
    return res.text();
}
